...
class PSPNet(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='resnet152', pretrained=True, use_aux=True, freeze_bn=False, freeze_backbone=False, use_biomass_head=True):
        ...
        self.use_biomass_head = use_biomass_head
        
        #if self.use_biomass_head:
            ...
            # Agri extra layers
            # linear dry bio mass regression for 3 classes: grass, clover, weeds
            #https://github.com/j96w/DenseFusion/blob/master/lib/pspnet.py
            #print("m_out_sz "+str(m_out_sz))
            #print("num_classes "+str(num_classes))
            
            #self.linear_biomass = nn.Linear((m_out_sz//4)*num_classes, 3)
            #self.linear_herbage = nn.Linear([8, 11520], 1)
            #self.linear_height = nn.Linear([8, 11520], 1) 
            #self.linear_biomass = nn.Sequential(
            #nn.Conv2d(m_out_sz//2, m_out_sz//4, kernel_size=3, padding=1, bias=False),
            #norm_layer(m_out_sz//4),
            #nn.ReLU(inplace=True),
            #nn.Dropout2d(0.1),
    ...
    def forward(self, x):
        ...
        output_m = self.master_branch(x)
        # output_m torch.Size([8, 5, 48, 48])

        # flatten the tensor where output_m.size(0) is first diamension
        f = output_m.view(output_m.size(0), -1)
        
        output = F.interpolate(output_m, size=input_size, mode='bilinear')
        output = output[:, :, :input_size[0], :input_size[1]]

        #if self.use_biomass_head:
            #print("xxx "+str(output_m.size()))
            # MEMEMEM torch.Size([8, 11520])

            # Agri extra layers
            # linear dry bio mass regression for 4 classes: grass, clover, weeds, soil
            #c = self.linear_biomass(x_aux)
    ...
    def save_onnx_model(self):
            ...
            dummy_input = Variable(torch.randn(8, 3, 400, 400, device='cuda'))
            model_filename = "./pspnet.onnx"
...
class PSPDenseNet(BaseModel):
    def __init__(self, num_classes, in_channels=2, backbone='densenet201', pretrained=True, use_aux=True, freeze_bn=False, **_):
